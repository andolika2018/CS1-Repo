...
list_remove b nil = nil
list_remove b (h::t) = ite (eql b h)
                       t
                       (h::(list_remove b t))
...
set_member b (mkSet l) = member b l
-------------------------------------
...
list_difference (h::t) l1 = ite (member h l1)
                              (list_difference t l1)
                              (h::(list_difference t l1))
list_difference nil b = nil
list_difference b nil = b
...
set_forall p (mkSet l) = ite (member (false) (map p l))
                         false
                         true
...
--Pair elt takes an elt h and a list b and rturns a list of all pairs,
  --(h,e) where e is an element of b
pairelt: a -> list b -> list (pair a b)
pairelt v nil = nil
pairelt v (h::t) = (mkPair v h)::(pairelt v t)
...
-------------------------------------
--Here you need to provide an implementation of set_toString. Format a set like we did with a list, but use curly braces instead of square brackets. Make sure that commas appear in all and only the right places.
